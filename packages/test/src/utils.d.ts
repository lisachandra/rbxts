...
/**
 * Represents a mock wrapper around an original object instance. Provides transparent property
 * access via Luau metatables while tracking which methods should rebind `self` to the original
...
interface MockInstance<T extends object> {
	...
	__instance__: T;
	/**
	 * Controls whether methods accessed through the mock are rebound so that `self` refers to
	 * `__instance__` rather than the mock proxy. The Luau implementation wraps the method as
...
/**
 * Signature for a mocked method produced by the runtime mock proxy. The `this` context is void (the
 * callable is a free function), `object` receives the original instance, and the remaining args and
 * return type are forwarded from the original method.
 *
...
/**
 * Recursively transforms a type so that every function-valued property becomes a {@link MockMethod},
 * while non-function values and nested objects are left as-is (with nested objects further
 * recursed).
 *
...
/**
 * Combines a {@link MockInstance} wrapper with Jest's deep mock infrastructure so every nested
 * property is recursively mocked. Object-typed properties are themselves wrapped via
 * {@link TestRuntimeUtils.createMockInstance}.
 *
 * @template T - The mocked object type (typically `jest.MockedObjectDeep<...>`).
 ...
 * 2. `jest.MockedObjectDeep<MockedObjectWithMethodsDeep<T>>` — Jest mock tracking for every function
 *    property.
 * 3. `__mockValue__` — a hook that lets test authors intercept property access and supply a custom
 *    mock value, bypassing the default auto-mocking logic.
 *
 * @remarks
 *   When a property is accessed for the first time, the proxy calls `__mockValue__(self, key,
 *   currentValue)`. If it returns `useMocked == true`, the returned `mockedValue` is used directly.
 *   Otherwise, the default auto-mocking logic runs:
 *
 *   - Functions are wrapped in `jest.fn(currentValue)`.
 *   - Plain tables and userdata are recursively wrapped via `mockOnRuntime(jest,
 *     createMockInstance(value))`.
 *   - All other values pass through unchanged.
 *   - The result is cached on the proxy so subsequent accesses are direct property reads.
 *
 * @example
 * 	```ts
...
> & {
	/**
	 * Intercepts property access on the mock proxy. Called automatically by the Luau `__index`
	 * metamethod with `(self, key, currentValue)`.
	 *
	 * - Return `true, replacementValue` to bypass auto-mocking and use `replacementValue` directly.
	 * - Return `false, nil` (the default) to run the standard auto-mocking logic.
	 *
...
declare const TestRuntimeUtils: {
	/**
	 * Creates a mock wrapper around an instance. Property reads pass through to the original
	 * object; function properties are optionally rebound so `self` refers to `__instance__` rather
	 * than the mock proxy.
	 *
	 * @example
	 * 	```ts
	 ...
	 * @param mockSelf - Whether to rebind methods' `self` to the instance. Defaults to `true`. Pass
	 *   a `Record<keyof T, boolean>` to selectively control which keys are rebound.
	 * @returns A `MockInstance<T>` merged with a writable partial of `T`, allowing tests to set or
	...
	/**
	 * Traverses a Roblox Instance hierarchy to find a `ModuleScript` by successive `WaitForChild`
	 * calls. Useful for locating compiled modules inside nested container folders during tests.
	 *
	 * @example
	 * 	```ts
	 * 	// Find container.SubFolder.MyModule
	 * 	const mod = TestRuntimeUtils.getModuleByTree(container, ["SubFolder", "MyModule"]);
	 * 	```;
	 *
	 * @param root - The starting `Instance` (e.g. `script.Parent`).
	 * @param parts - Ordered array of child names to traverse (passed to `Instance:WaitForChild`).
	 * @returns The `Instance` at the end of the hierarchy. While the type declares `ModuleScript`,
	 *   the Luau implementation returns the raw result of the final `WaitForChild` call.
	 */
	getModuleByTree: (root: Instance, parts: Array<string>) => ModuleScript;

	/**
	 * Whether the current execution is inside a Jest test runner. Set to `_G.__TEST__` by the setup
	 * script.
	 *
	 * @example
	 * 	```ts
	...
	/**
	 * Promotes a {@link MockInstance} to a fully-instrumented `MockOnRuntime`. Function properties
	 * accessed through the returned proxy are automatically wrapped with `jest.fn()`, and a
	 * `__mockValue__` hook is added for per-key overrides.
	 *
	 * Nested tables and userdata are recursively wrapped. Values are cached on the proxy so each
	 * key is only resolved once.
	 *
	 * @example
	 * 	```ts
	 * 	import { jest } from "@rbxts/jest-globals";
	 ...
	 * 	```;
	 *
	 * @param jestModule - The `jest` namespace (from `@rbxts/jest-globals`).
	...
	/**
	 * Finds and resets the active roblox-ts `RuntimeLib`. Iterates all `_G` entries, clears any
	 * keyed by `ModuleScript`, and calls `RuntimeLib.reset()` on the first runtime found.
	 *
	 * @example
	 * 	```ts
	 ...
	 * 	```;
	 *
	 * @param clean - If `true`, additionally destroys all children of `Workspace` except `Terrain`
	 *   and `Camera`. Useful for cleaning up spawned instances between test suites.
	...
	/**
	 * Restores all `jest.fn()` mocks created by `mockOnRuntime` back to their original
	 * implementations. For each mock, calls `mockRestore()` followed by
	 * `mockImplementation(original)`.
	 *
	 * Call this in `afterEach` or between test cases to prevent cross-test contamination.
	 *
	 * @example
	 * 	```ts
	 * 	afterEach(() => {
