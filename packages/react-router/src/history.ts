...
/**
 * Manages a linear navigation history with support for pushing,
 * replacing, and navigating between entries.
 *
 * @remarks
 * Fires {@link History.onChanged} whenever the current location
 * changes. The initial index is clamped to `[1, entries.length]`.
 */
export class History {
	...
	/**
	 * Signal fired with `(path, state)` each time the active location
	 * changes.
	 */
	 ...
	 * @param initialIndex - 1-based index of the initial active entry.
	...
	/**
	 * Moves the current index by the given offset, clamped to valid
	 * bounds.
	 *
	...
	/**
	 * Navigates to the previous history entry. Convenience for
	 * `go(-1)`.
	 */
	public goBack(): void {
		this.go(-1);
	...
	/**
	 * Navigates to the next history entry. Convenience for
	 * `go(1)`.
	 */
	...
	/**
	 * Adds a new entry after removing any forward history, then
	 * navigates to it.
	 *
	...
	/**
	 * Replaces the current entry without changing the history length,
