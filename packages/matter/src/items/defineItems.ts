 ...
 *
 * @remarks
 *   Items can be organized hierarchically through {@link children}. Leaves are defined by providing
 *   {@link defaultData} and optionally {@link serdes}, while branches only need {@link children} and
 *   optional metadata like {@link description} and {@link image}.
...
export interface ItemDefinitionConfig<TData extends object = object> {
	 ...
	 *
	 * 1. **Type-level**: Defines the shape of `data` in `Components.Item`, enabling type-safe access
	 *    to item fields (e.g., `item.data.damage`).
	...
	image?: string;
	/**
	 * Keys to exclude from network replication (kept server-side only). These will be set in
	 * `privateDefinitions`.
	 *
	...
	/**
	 * Serializer/deserializer for the item's data payload. Defines which fields are replicated over
	 * the network.
	 */
...
/**
 * Creates item definitions, serdes, descriptions, and registers item IDs from a single unified
 * configuration object.
 *
 * This replaces the need to manually keep `itemDefinitions`, `serdes`, `descriptions`, and the item
 * registry in sync.
 *
 * @example
 * 	```ts
...
export function defineItems(config: Record<string, ItemDefinitionConfig>): void {
	populateDefinitions(itemDefinitions, config);
	populatePrivateDefinitions(config);

	// serdes and descriptions are populated separately to maintain their type-safe satisfies expressions
	populateSerdes(serdes, config);
	populateDescriptions(descriptions, config);
 ...
 *
 * Only PascalCase keys (hierarchy) are used for traversal. Default data values are non-PascalCase
 * and become the leaf data type.
 */
