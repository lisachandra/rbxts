...
type MatterComponentFactory = (data?: object) => Component<object>;

export type EntityLookupComponentName = string;
...
const defaultInstanceComponents: Array<EntityLookupComponentName> = [
	"Profile",
	"Items",
	"Node",
];
...
export function getEntityLookupComponent(name: EntityLookupComponentName): MatterComponentFactory | undefined {
	return (Components as never as Record<string, MatterComponentFactory | undefined>)[name];
}
...
export function getEntityComponentByName(
	...
	return component !== undefined ? (world.get(entityId, component as never) as Component<object> | undefined) : undefined;
}
 ...
 *
 * Defines which components are used to identify an entity as "streamable"
 * (i.e., eligible for network streaming between server and client).
 *
 * **Important:** Streamable components MUST include a `model` key of type
 * `Instance`. The streaming system (`clientStreamer.ts`) casts the matched
 * component to `Component<{ model: Instance }>` to track the associated
 * Roblox instance during streaming events. Components without a `model`
 * field will cause a type mismatch at the call site.
 *
 * @example
 * ```ts
...
const defaultStreamableComponents: Array<EntityLookupComponentName> = [
	"Items",
];
 ...
 *
 * Each component name listed here must correspond to a Matter component
 * whose data shape includes a `model: Instance` key. This constraint is
 * required by `clientStreamer.ts`, which casts the resolved component to
 * `Component<{ model: Instance }>` for tracking the associated Roblox
 * instance.
 *
...
/**
 * Resolves a streamable component for the given entity.
 * Returns the first matching component from the configured streamable component list.
 *
