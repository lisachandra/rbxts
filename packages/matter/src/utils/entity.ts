...
import {
	getEntityComponentByName,
	getEntityHumanoidComponent,
	getEntityInstanceComponent,
} from "../entityLookup";

/**
 * Checks if an entity is alive in the world by verifying it exists and has a valid Humanoid.
 *
...
/**
 * Recursively searches an instance's ancestry for a "clientEntityId" attribute.
 *
...
/**
 * Recursively searches an instance's ancestry for a "serverEntityId" attribute.
 *
 * @param instance - The instance to start the search from.
...
export function ownsEntity(entityId: AnyEntity): boolean {
	const npc = getEntityComponentByName(store.world, entityId, "NPC" as ComponentKey) as
		| undefined
		| { owner?: Player };
...
/**
 * Retrieves the primary Roblox object associated with a given Matter component.
 *
...
/**
 * Gets the primary object associated with an entity using the configured entity lookup component
 * order.
 *
 * @param entityId - The ID of the entity in the world.
 * @returns The instance representing the entity, or undefined if not found or no suitable component
 *   exists.
 */
export function getEntityObject(entityId: AnyEntity = -1 as AnyEntity): N<PVInstance> {
	...
	return world !== undefined
		? getComponentObject(getEntityInstanceComponent(world, entityId))
		: undefined;
}

/**
 * Gets the world position of an entity using the configured entity lookup component order.
 *
 * @param entityId - The ID of the entity in the world.
 * @returns The position of the entity, or undefined if the entity or its position could not be
 *   determined.
 */
export function getEntityPosition(entityId: AnyEntity = -1 as AnyEntity): N<Vector3> {
	...
	return world !== undefined
		? getComponentPosition(getEntityInstanceComponent(world, entityId))
		: undefined;
}
 ...
 *
 * @param entityId - The ID of the entity in the world.
 * @param nonStrict - If true, returns the Humanoid even if it's dead or missing a RootPart.
