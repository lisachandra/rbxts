...
/**
 * Augmentable interface mapping user-type keys to their Centurion
 * argument type name strings.
 *
 * @remarks
 * External packages and consumers should augment this interface via
 * `declare module` so that all centurion types across the entire
 * codebase are discoverable from a single import location —
 ...
 *
 * Mirrors `CenturionType` from `@rbxts/centurion`. Consumers and external
 * packages extend this object via {@link registerCenturionType}, so
 * `CenturionUserType.Foo` resolves to `"foo"` at both compile time and
 * runtime — no manual spreading needed.
 *
 * @example
 * ```ts
...
/**
 * Registers a custom Centurion argument type at runtime, adding it
 * directly to {@link CenturionUserType}.
 *
 * Pair with a `declare module` augmentation of {@link CenturionUserTypes}
 * so the key is known at compile time as well.
 *
 * @param key - The type key to register.
 * @param name - The string identifier used in Centurion command argument
 *   definitions (e.g. the `type` field).
 *
 * @example
 * ```ts
 * declare module "@lisachandra/platform/out/centurion/type" {
