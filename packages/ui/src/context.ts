...
import { FetchValue } from "@lisachandra/react-template"

/**
 * Provides methods for scaling UI elements based on viewport size. Offers
 * various rounding options and UDim conversion utilities. Uses a base
 * resolution and dominant axis to calculate scaling factor. Can be called
 * directly as a function for rounded scaling.
 */
export interface PxWithMethods {
	...
	/**
	 * Scales `pixels` and rounds the result up. Useful for ensuring minimum
	 * sizes.
	 */
	ceil: (value: number) => number;

	/**
	 * Scales `pixels` and rounds the result to the nearest even number. Useful
	 * for ensuring consistent alignment.
	 */
	even: (value: number) => number;

	/**
	 * Creates a reactive value that dynamically scales a value. Useful for
	 * binding UI template element sizes to the viewport. Uses rounded scaling
	 * by default, but accepts a custom scaling function.
	 *
	 * @example
	 *
	 * ```tsx
	...
	/**
	 * Scales `pixels` and rounds the result down. Useful for preventing
	 * overflow.
	 */
	floor: (value: number) => number;

	/**
	 * Scales a UDim's offset value. Accepts a custom scaling function or
	 * defaults to rounded scaling. Can also be partially applied for deferred
	 * scaling of UDim objects.
	 *
	 * @example
	 *
	 * ```ts
	...
	fromUDim: {
		...
		/**
		 * Partial application: returns a function that scales UDim offset
		 * values using the provided scaling function.
		 */
	...
	/**
	 * Scales a UDim2's offset values. Accepts a custom scaling function or
	 * defaults to rounded scaling. Can also be partially applied for deferred
	 * scaling of UDim2 objects.
	 *
	 * @example
	 *
	 * ```ts
	 ...
	 *
	 * // With a custom scaling function
	 * const customScaledUDim = px.fromUDim2(
	...
	fromUDim2: {
		...
		/**
		 * Partial application: returns a function that scales UDim2 offset
		 * values using the provided scaling function.
		 */
