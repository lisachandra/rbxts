...
import type { ConnectionLike, EventLike } from "./type";
...
export function getHumanoid(model?: Instance, nonStrict = false): N<Humanoid> {
	...
	if (!nonStrict && (humanoid.GetState() === Enum.HumanoidStateType.Dead || !humanoid.RootPart)) {
		return;
	}
...
export async function loadAnimation(
	...
	track.Priority =
		Enum.AnimationPriority[
			animation.GetAttribute<Enum.AnimationPriority["Name"]>("priority") ??
				track.Priority.Name
		];
...
export async function tween(
	...
	return new Promise<void>((resolve, _reject, onCancel) => {
		onCancel(() => {
			lazyDisconnect(connection);
		});
...
export async function waitForFirst(
	...
	const slots = signals.map(async (signal) => {
...
export function getInstanceByTree<T extends Instance>(
	root: Instance,
	parts: Array<string>,
): T | undefined {
	let current = root;
