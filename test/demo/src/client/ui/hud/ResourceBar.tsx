...
export function ResourceBar(): JSX.Element {
	...
	return (
		<frame
			BackgroundColor3={Color3.fromRGB(31, 48, 35)}
			BackgroundTransparency={0.15}
			Size={UDim2.fromOffset(px(280), px(100))}
		>
			...
			<textlabel
				...
				Size={UDim2.fromOffset(px(260), px(22))}
				Text={`${carry?.kind ?? "Nothing"} x${tostring(carry?.amount ?? 0)}`}
				TextColor3={Color3.fromRGB(227, 235, 222)}
				TextSize={px(16)}
			/>
			<textlabel
				BackgroundTransparency={1}
				Font={Enum.Font.Gotham}
				Size={UDim2.fromOffset(px(260), px(22))}
				Text={`Nearby Task: ${prompt?.text ?? "Walk near a pickup."}`}
				TextColor3={Color3.fromRGB(196, 220, 173)}
				TextSize={px(14)}
				TextWrapped={true}
			/>
		</frame>
