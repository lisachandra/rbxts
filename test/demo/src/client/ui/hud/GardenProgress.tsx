...
export function GardenProgress(): JSX.Element {
	...
	return (
		<frame
			BackgroundColor3={Color3.fromRGB(26, 60, 35)}
			BackgroundTransparency={0.1}
			Size={UDim2.fromOffset(px(240), px(110))}
		>
			...
			<textlabel
				...
				Size={UDim2.fromOffset(px(220), px(24))}
				Text="Garden Health"
			...
			<textlabel
				...
				Size={UDim2.fromOffset(px(220), px(22))}
				Text={`${tostring(progress?.health ?? 0)}%`}
				TextColor3={Color3.fromRGB(157, 255, 155)}
				TextSize={px(16)}
			/>
			<textlabel
				BackgroundTransparency={1}
				Font={Enum.Font.Gotham}
				Size={UDim2.fromOffset(px(220), px(22))}
				Text={`Restored Plots: ${tostring(progress?.restoredPlots ?? 0)}/${tostring(progress?.totalPlots ?? 0)}`}
				TextColor3={Color3.fromRGB(230, 236, 217)}
				TextSize={px(14)}
			/>
			<textlabel
				BackgroundTransparency={1}
				Font={Enum.Font.Gotham}
				Size={UDim2.fromOffset(px(220), px(22))}
				Text={`Harvested: ${tostring(progress?.harvested ?? 0)}`}
				TextColor3={Color3.fromRGB(255, 221, 138)}
				TextSize={px(14)}
			/>
		</frame>
	);
